...
<body>
<table id="table-dept" title="<i class='icon-save'/><m:info name='机构管理'/>&nbsp;" style="width:100%;height:400px"></table>
    ...
    function actionOver(code){
        switch(code){
            ...
            <shiro:hasPermission name="dept:save:add">
                addChangedeptTree();
                $('#dialog-dept').dialog({title: "<m:info name='新增菜单'/>"});
            ...
            <shiro:hasPermission name="dept:save:edit">
                if(editChangedeptTree()) {
                    $('#dialog-dept').dialog({title: "<m:info name='资源编辑'/>"});
                    $('#dialog-dept').dialog('open');
                }else{
...
<script>
    ...
    function editChangedeptTree(){
        ...
        if(row != null) {//有选择资料,将资料值初始化到表单
            deptTreeSelect(row.parentid);
            setFormRow(row);
            $('#dialog-dept').dialog({title: "<m:info name='请选择...'/>"});
            $('#dialog-dept').dialog('open');
            return true;
        }else{//未选择资料,默认初始化值
            return false;
